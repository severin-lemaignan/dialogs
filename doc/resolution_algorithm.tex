...
\PROCEDURE{GenerateDescriptionForNominalGroup}{group} 
   noun \GETS \CALL{GetNoun}{group} \\ 
   \IF \NOT \CALL{Ontology.Lookup}{noun} \in (Instances) \STMTNUM{7.5em}{st.lookup} \THEN
   ...
   \FOREACH nouncmpl \in nounComplements \DO
     \mathcal{D} = \mathcal{D} + {\CALL{GenerateDescriptionForNominalGroup}{nouncmpl}}\\
   
   
   \\  
   relativeClauses \GETS \CALL{GetSubordinateRelativeClause}{group} \\
   \FOREACH rel \in relativeClauses \DO
     \mathcal{D} = \mathcal{D} + {\CALL{GenerateDescriptionForNominalGroup}{rel}}\\
     \mathcal{D} = \mathcal{D} + {\CALL{GenerateDescriptionForVerbalGroup}{rel}}\\
