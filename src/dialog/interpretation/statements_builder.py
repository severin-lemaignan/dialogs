...
def dump_resolved(sentence, current_speaker, current_listener):
	def resolve_ng(ngs, builder):		
		for ng in ngs:
			...
			#existing ID with lookup on the ontology 
				...
				onto_fiat = ''
				try:
					onto_fiat =  ResourcePool().ontology_server.find('?concept', ['?concept belongsTo ?a', '?concept rdf:type Car','?a belongsTo id_danny'])
				except AttributeError: #the ontology server is not started of doesn't know the method
					pass
...
class TestStatementBuilder(unittest.TestCase):
	...
	def setUp(self):
		...
		try:
			ResourcePool().ontology_server.add(['SPEAKER focusesOn another_cube',
