...
class QuestionHandler(object):
    ...
    def _set_situation_id(self, statements):
        ...
        if find_id:
            ...
            if sit_id:
                logger.debug(
                    "\t/Found a staticSituation matching the yes_no_question query to be checked: " + str(sit_id))
                for s in statements:
            ...
            for sv in self._sentence.sv:
                for verb in sv.vrb_main:
                    ...
                    # Case of action verb with a passive behaviour
                        ...
                        stmts.append(
                            sn.id + ' ' + ResourcePool().action_verb_with_passive_behaviour[verb.lower()] + ' ?concept')
